...
| macOS 26 Tahoe 26.4.1 | Tested; failures and evidence gaps remain | 25E253 on an Apple M2 Pro (arm64), Command Line Tools 26.6, and Swift 6.3.3. The initial `b2738b4` pass verified build, install, repository checks, 219/219 smoke fixtures, configuration recovery, six fullscreen dismissal paths, and the window-fitting defense for closed [#2](https://github.com/matthewsundling/cowsaver/issues/2). Candidate `32375f8` later built and installed on the same host; `make test` passed 329 tests in 28 suites, the 43-test settings suite passed, diagnostic and repository checks passed, and smoke again matched 219/219 fixtures. Its corrected settings Preview painted selected backgrounds correctly in both System Settings Options and standalone `--configure` ([#65](https://github.com/matthewsundling/cowsaver/issues/65)). Its corrected GPU diagnostic identified `AGXAcceleratorG14X` as unified and exited 0 because the discrete comparison does not apply ([#68](https://github.com/matthewsundling/cowsaver/issues/68)); that classification is not a resource-use or power measurement. Remaining Tahoe findings are clock/date overlap in all three recorded clock-on activation paths ([#66](https://github.com/matthewsundling/cowsaver/issues/66)), uncharacterized slow standalone settings scrolling ([#67](https://github.com/matthewsundling/cowsaver/issues/67)), and Options reopening failures whose host/package/race cause remains unclassified ([#1](https://github.com/matthewsundling/cowsaver/issues/1)). No external display or standalone `--idle 10` result was recorded. |
...
macOS 26 Tahoe has been tested on Apple silicon. The 26.4.1 passes directly exercised the window-fitting defense, and the original timer-clipping failure is resolved. Candidate `32375f8` also corrected the settings Preview background in both settings front ends and correctly classified the M2 Pro GPU as unified. Tahoe is still not supported: standalone settings scrolling is slow but uncharacterized; System Settings reproducibly loses Options after `Cancel → Done → Screen Saver → Options` for a cause not yet isolated; and Tahoe's clock/date group overlaps Cowsaver content in every recorded clock-on activation path. The completed field evidence covers one built-in display, not external-display behavior or app display reconciliation. The unified-GPU classification result does not measure whether Cowsaver changes baseline resource use.

The configuration file remains the supported configuration path on tested systems. A fresh Tahoe Command Line Tools installation could build and install Cowsaver but could not run `make test` because it lacked Swift Testing; `make test` reports that limitation clearly, and `make smoke` passed all 219 framework-free golden fixtures. After full Xcode 26.6 was installed, candidate `32375f8` passed the full 329-test suite on the exact Tahoe 26.4.1 host. See closed [#4](https://github.com/matthewsundling/cowsaver/issues/4) and the [Tahoe reconciliation tracker](https://github.com/matthewsundling/cowsaver/issues/69). Each verification pass follows the [macOS compatibility checklist](release-checklist.md), and every completed run belongs in the table above. That checklist records field evidence; the [maintainer release procedure](releasing.md) covers release state transitions separately.
